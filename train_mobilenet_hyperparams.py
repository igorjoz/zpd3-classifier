...
def write_report(path, config, split_summary, history, top_checkpoints, test_metrics):
    ...
    lines.extend(
        [
            ...
            "",
            "Checkpointy `MODELS_IMPROVED` wybrano wylacznie na podstawie validation macro F1 "
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("hyperparams_outputs") / "mobilenet_v3_large"
    )
    parser.add_argument("--models-dir", type=Path, default=Path("MODELS_IMPROVED/hyperparams"))
